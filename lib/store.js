...
(function (module) {
    ...
    function parseKey(url, content, result) {
        ...
        if (matched) {
            ...
            //result[id] = version;
            result[id] = parse(content.Headers.etag);
        }
    }
    ...
    function versionByID(url, cb) {
        ...
        return hidden.s3.ListObjects({
            ...
            hidden.logger.info("s3.ListObjects", url, Date.now() - start);
    ...
    function get(url, cb) {
        ...
        return hidden.s3.GetObject({
            ...
            hidden.logger.info("s3.GetObject", url, Date.now() - start);
    ...
    function read(url, cb) {
        return get.call(this, url, function(err, content, etag) {
            return cb(err, content.vals, etag);
        });
    }
    ...
    function save(url, data, cb) {
        ...
        return hidden.s3.PutObject({
            ...
            hidden.logger.info("s3.PutObject", url, Date.now() - start);
            ...
            return cb(undefined, data, etag);
        });
    }

    function create(url, data, cb) {
        return save.call(this, url, {
            vals: data,
            mods: {}
        }, function(err, data, etag) {
            return cb(err, data && data.vals, etag);
        });
    }
    ...
    function remove(url, cb) {
        ...
        return hidden.s3.DeleteObject({
            ...
            hidden.logger.info("s3.DeleteObject", url, Date.now() - start);

            return cb(err);
        });
    }

    function merge(logger, url, data, timestamp, modifications) {
    ...
    function update(url, timestamp, modifications, cb) {
        var hidden = this;

        return get.call(hidden, url, function (err, data, etag) {
            merge(hidden.logger, url, data || {}, timestamp, modifications);
            save.call(hidden, url, data, function() {
                return cb(err, data.vals, etag);
            });
        });
    }
    ...
    module.exports = function (logger, properties, cb) {
        return connect(properties, logger, function (err, hidden) {
            ...
            return cb(undefined, {
                ...
                create:function () {
                    return create.apply(hidden, arguments);
                },
                read:function () {
                    return read.apply(hidden, arguments);
                },
                update:function () {
                    return update.apply(hidden, arguments);
                },
                remove:function () {
